    ...
    def __init__(self, w_directory,k_mer_val):
        ...
        for filename in os.listdir(self.working_directory):
            if filename.endswith(".fa") or filename.endswith(".fasta"):
                ...
                ID_seq_list = list(zip([binary_seq_list[index] for index in range(len(binary_seq_list)) if index % 2 == 0],
                                       [binary_seq_list[index] for index in range(len(binary_seq_list)) if index % 2 != 0]))
                pattern_result = self.find_pattern(ID_seq_list)
                for element in pattern_result:
                    self.pattern_freq[element.split(" ")[0]] = int(element.split(" ")[1])
                sorted_pat_freq = dict(sorted(self.pattern_freq.items(), key=lambda item: item[1], reverse=True))
                with open(f'{input_file.split(".fa")[0]}_frequency.txt', 'w') as writefrequencyfile:
                    writefrequencyfile.writelines('{} - {} \n'.format(Pattern, Frequency) for Pattern, Frequency in sorted_pat_freq.items())
                t2 = time.time()
    ...
    def create_binary(self, seq_data):
        ...
        for index in range(0, len(seq_data)):
            if index % 2 != 0 and len(seq_data[index]) > 50:
                ...
                seq_binary_list.append(seq_data[index].translate(str.maketrans({'A': str(0), 'T': str(0), 'G': str(1), 'C': str(1), 'a': str(0), 't': str(0), 'g': str(1), 'c': str(1)})))
