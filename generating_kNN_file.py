    ...
    def create_binary(self, seq_data):
        ...
        for index in range(0, len(seq_data)):
            if index % 2 != 0 and len(seq_data[index]) > 50:
                ...
                output.append(seq_data[index].translate(str.maketrans({'A': str(0), 'T': str(0), 'G': str(1), 'C': str(1), 'a': str(0), 't': str(0), 'g': str(1), 'c': str(1)})))

    
        output_list = list(zip([output[x] for x in range(0, len(output)) if x % 2 == 0],
...
if __name__ == '__main__':
    ...
    directory = input('Please give the path of directory containing CDS and ncRNA files: ')
